...
When invoked, the ``--network`` option searches the apptainer
configuration directory (commonly
``/usr/local/etc/apptainer/network/``) for the cni configuration file
...
network configurations through options in ``apptainer.conf``.
