...
.. code::
   ...
   $ SINGULARITYENV_CUDA_VISIBLE_DEVICES=0 apptainer run --nv tensorflow_latest-gpu.sif
...
-  ``nvidia-container-cli`` must be installed on your host. Its path must
   be set in ``apptainer.conf``. This value will be set at build time if
...
from the environment where ``apptainer`` is run. {Singularity} does
