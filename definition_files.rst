...
process is equivalent to running ``apptainer verify`` on the bootstrap
image.
...
``apptainer test`` will exit with an error such as ``samtools: command
not found``.
...
via the ``apptainer run`` command or by executing the container
...
``/.apptainer.d/labels.json`` within your container. The general
